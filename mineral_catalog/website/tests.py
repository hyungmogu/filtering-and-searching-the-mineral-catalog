...
# MODEL TEST
class MineralModelTest(TestSetup):
    def test_mineral_creation(self):
        # query all objects in database
        mineral = Mineral.objects.get(pk=1)

        # the number of items in database must be 2
        self.assertEqual(Mineral.objects.count(), 2)
...
# VIEW TEST
class MineralHomePageTestCase(TestSetup):
    ...
    def test_return_layouthtml_as_template_used(self):
        self.assertTemplateUsed(self.resp_home, 'website/layout.html')

    def test_return_homehtml_as_template_used(self):
        self.assertTemplateUsed(self.resp_home, 'website/home.html')

    # check and see if it passes the list of mineral names to view
    def test_return_list_of_mineral_names(self):

        self.assertIn(self.mineral1, self.resp_home.context['minerals'])
        self.assertIn(self.mineral2, self.resp_home.context['minerals'])


class MineralDetailPageTestCase(TestSetup):
    '''Tests for the Article detail view'''
    def test_return_status_okay(self):
        self.assertEqual(self.resp_detail.status_code, 200)

    def test_return_layouthtml_as_template_used(self):
        self.assertTemplateUsed(self.resp_detail, 'website/layout.html')

    def test_return_detailhtml_as_template_used(self):
        self.assertTemplateUsed(self.resp_detail, 'website/detail.html')

    def test_return_info_of_added_mineral(self):
        self.assertContains(self.resp_detail, self.mineral1.name)


# FILTER TEST
class MineralFilterByGroupTestCase(TestSetup):
...
class MineralFilterByTextTestCase(TestSetup):
...
class MineralFilterByFirstLetterTestCase(TestSetup):
